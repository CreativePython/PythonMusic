"""pem.config -- Manage PEM configuration information.
...
pem. This is to allow PEM to continue to function in spite of errors in
...
class InvalidConfigType(Exception):
    "Unknown value type requested from / passed to a pemConf getter or setter."
class InvalidConfigSet(Exception):
    "A config 'set' other than 'user' or 'default' was named."
class InvalidTheme(Exception):
    "A highlight theme name that isn't a built-in or a user theme was named."

class PemConfParser(ConfigParser):
    """
    A ConfigParser specialised for pem configuration file handling
    """
...
class PemUserConfParser(PemConfParser):
    """
    PemConfigParser specialised for user configuration handling.
    """
...
class PemConf:
    """Hold config parsers for all pem config files in singleton instance.

    Default config files, self.defaultCfg --
        for config_type in self.config_types:
            (pem install dir)/config-{config-type}.def

    User config files, self.userCfg --
        for config_type in self.config_types:
        (user home dir)/.pemrc/config-{config-type}.cfg
    """
    ...
    def CreateConfigHandlers(self):
        ...
        pemdir = os.path.dirname(__file__)
        self.userdir = userdir = '' if pem.testing else self.GetUserCfgDir()
        for cfg_type in self.config_types:
            self.defaultCfg[cfg_type] = PemConfParser(
                os.path.join(pemdir, f'config-{cfg_type}.def'))
            self.userCfg[cfg_type] = PemUserConfParser(
    ...
    def GetUserCfgDir(self):
        ...
        if userDir == "~": # still no path to home!
            # traditionally PEM has defaulted to os.getcwd(), is this adequate?
            userDir = os.getcwd()
    ...
    def GetOption(self, configType, section, option, default=None, type=None,
        ...
        try:
            ...
            warning = ('\n Warning: config.py - PemConf.GetOption -\n'
    ...
    def GetThemeDict(self, type, themeName):
        ...
        # yet used by pem, to allow for their use in the future.
        ...
        for element in theme:
            if not (cfgParser.has_option(themeName, element) or
                ...
                warning = ('\n Warning: config.PemConf.GetThemeDict'
    ...
    def current_colors_and_keys(self, section):
        ...
        pem.config-main.def ('default') includes these sections
        ...
        because setting 'name' to a builtin not defined in older PEMs
        to display multiple error messages or quit.
        See https://bugs.python.org/issue25313.
        When default = True, 'name2' takes precedence over 'name',
        while older PEMs will just use name.  When default = False,
        ...
        if name:
            ...
            if source[cfgname].has_section(name):
                return name
        return "PEM Classic" if section == 'Theme' else self.default_keys()
    ...
    def GetExtensions(self, active_only=True,
        ...
        for extn in userExtns:
            if extn not in extns: #user has added own extension
                extns.append(extn)
        for extn in ('CodeContext',
                     'FormatParagraph','ParenMatch',
                     'AutoComplete', 'CallTip', 'AutoExpand'):
            if extn in extns:
                extns.remove(extn)
    ...
    def IsCoreBinding(self, virtualEvent):
        """Return True if the virtual event is one of the core pem key events.
...
# and copied in function below.
    ...
    former_extension_events = {  #  Those with user-configurable keys.
        '<<flash-paren>>', '<<format-paragraph>>',
         '<<run-module>>', '<<check-module>>', '<<zoom-height>>',
         '<<run-custom>>', '<<run-selection>>', '<<run-current-line>>',
    ...
    def GetCoreKeys(self, keySetName=None):
        ...
        keyBindings={
            ...
            '<<close-window>>': ['<Control-Key-w>', '<Control-Key-W>'],
            # '<<quit>>': ['<Control-q>'],
            # '<<save-all-windows>>': ['<Control-Alt-s>'],
            ...
            '<<open-module>>': ['<Alt-m>'],
            '<<open-new-window>>': ['<Control-n>', '<Control-t>'],
            ...
            '<<find>>': ['<Control-f>'],
            '<<replace>>': ['<Control-Shift-Key-F>'],
            '<<goto-line>>': ['<Alt-g>'],
            '<<smart-backspace>>': ['<Key-BackSpace>'],
            '<<newline-and-indent>>': ['<Key-Return>', '<Key-KP_Enter>'],
            '<<smart-indent>>': ['<Key-Tab>'],
            '<<indent-region>>': ['<Control-Key-bracketright>'],
            '<<dedent-region>>': ['<Control-Key-bracketleft>', '<Shift-Key-Tab>'],
            '<<comment-region>>': ['<Alt-Key-3>'],
            '<<uncomment-region>>': ['<Alt-Key-4>'],
            '<<toggle-comment>>': ['<Control-Key-slash>'],
            ...
            '<<run-module>>': ['<Control-Key-r>'],
            '<<run-custom>>': ['<Shift-Key-F5>'],
            '<<check-module>>': ['<Alt-Key-x>'],
            '<<run-selection>>': ['<Control-Shift-Key-R>'],
            '<<run-current-line>>': ['<Control-Shift-Key-L>'],
            '<<run-current-paragraph>>': ['<Control-Shift-Key-P>'],
            '<<stop-script>>': ['<Control-Key-F5>'],
            '<<open-python-shell>>': ['<Control-Shift-Key-C>'], 
        ...
        if keySetName:
            if not (self.userCfg['keys'].has_section(keySetName) or
                ...
                warning = (
                    '\n Warning: config.py - PemConf.GetCoreKeys -\n'
    ...
    def GetFont(self, root, configType, section):
        ...
        family = 'Courier New'  # *** PEM Edit: Always use Courier New
...
class ConfigChanges(dict):
    ...
    def __init__(self):
        ...
        for config_type in pemConf.config_types:
            self[config_type] = {}
    ...
    def save_option(config_type, section, item, value):
        ...
        Helper for save_all.
        """
        if pemConf.defaultCfg[config_type].has_option(section, item):
            if pemConf.defaultCfg[config_type].Get(section, item) == value:
                # The setting equals a default setting, remove it from user cfg.
                return pemConf.userCfg[config_type].RemoveOption(section, item)
        # If we got here, set the option.
        return pemConf.userCfg[config_type].SetOption(section, item, value)
    ...
    def delete_section(self, config_type, section):
        ...
        configpage = pemConf.userCfg[config_type]
    ...
    def clear(self):
        ...
        Called in save_all after saving to pemConf.
...
def _dump():  # htest # (not really, but ignore in coverage)
    ...
    dumpCfg(pemConf.defaultCfg)
    dumpCfg(pemConf.userCfg)
    print('\nlines = ', line, ', crc = ', crc, sep='')


if __name__ == '__main__':
    from unittest import main
    main('pem.pem_test.test_config', verbosity=2, exit=False)
