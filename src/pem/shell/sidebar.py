"""Line numbering implementation for PEM as an extension.
...
def get_displaylines(text, index):
   ...
   #                    "displaylines", return_ints=True)
    ...
    a, b = f"{index} linestart", f"{index} lineend"
    try:
        return text.count(a, b, "displaylines", return_ints=True)
    except TypeError:
        out = []
        text.count(a, b, "displaylines", out)
        return tuple(out)
...
class BaseSideBar:
    ...
    def update_colors(self):
        """Placeholder for color synchronization in subclasses."""
        pass
...
class LineNumbers(BaseSideBar):
    ...
    MIN_WIDTH = 4   # minimum line-number gutter width, in characters
    ...
    def init_widgets(self):
        ...
        self.sidebar_text = tk.Text(self.parent, width=self.MIN_WIDTH, wrap=tk.NONE,
                                    padx=2, pady=pady,
    ...
    def update_font(self):
        font = pemConf.GetFont(self.text, 'main', 'EditorWindow')
        self.sidebar_text['font'] = font

    def update_colors(self):
        """Update the sidebar text colors, usually after config changes."""
        # -------------------------------------------------------------
        # SUBSYSTEM: CUSTOM LINE NUMBER GUTTER
        # Apply a static, warm pastel peach (RGB: 255, 221, 170) to the 
        # gutter background to cleanly separate the code from the margins.
        # -------------------------------------------------------------
        colors = pemConf.GetHighlight(pemConf.CurrentTheme(), 'linenumber')
        foreground = colors['foreground']
        custom_background = '#ffddaa'

        self.sidebar_text.config(
            fg=foreground, bg=custom_background,
            selectforeground=foreground, selectbackground=custom_background,
            inactiveselectbackground=custom_background,
        )
    ...
    def update_sidebar_text(self, end):
        ...
        # Wide enough for the largest line number, but never below MIN_WIDTH.
        required_width = max(self.MIN_WIDTH, len(str(end)))
        current_width = int(float(self.sidebar_text['width']))

        # Only resize the widget if the required width actually changed (e.g., hitting line 1000)
        if required_width != current_width:
            self.sidebar_text['width'] = self._sidebar_width_type(required_width)
...
class WrappedLineHeightChangeDelegator(Delegator):
    "Percolator filter that calls back whenever an edit may change wrapped-line counts."
...
class ShellSidebar(BaseSideBar):
    ...
    def update_colors(self):
        # -------------------------------------------------------------
        # SUBSYSTEM: CONSOLE GUTTER SYNC
        # Synchronize the interactive console's prompt margin with the 
        # editor's custom peach background for visual cohesion.
        # -------------------------------------------------------------
        prompt_colors = pemConf.GetHighlight(pemConf.CurrentTheme(), 'console')
        foreground = prompt_colors['foreground']
        customBackground = '#ffddaa'
        
        self.colors = (foreground, customBackground)
        self.canvas.configure(background=customBackground)
        self.change_callback()
    ...
    def update_colors(self):
        ...
        self.colors = (foreground, custom_background)
        self.canvas.configure(background=custom_background)
        self.change_callback()


def _sidebar_number_scrolling(parent):  # htest #
    from pem.pem_test.test_sidebar import Dummy_editwin
    ...
    font = pemConf.GetFont(top, 'main', 'EditorWindow')
...
if __name__ == '__main__':
    ...
    main('pem.pem_test.test_sidebar', verbosity=2, exit=False)
